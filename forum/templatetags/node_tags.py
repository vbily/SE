...
def post_controls(post, user):
    ...
    if user.is_authenticated():
        ...
        if user.is_superuser or user.is_staff:
            if post.nis.approved or post.approved:
                controls.append(post_control(_('unapprove'), reverse('approve_post', kwargs={'id': post.id}),
                        command=True, confirm=True))
            else:
                controls.append(post_control(_('approve'), reverse('approve_post', kwargs={'id': post.id}),
                        command=True, confirm=True))
