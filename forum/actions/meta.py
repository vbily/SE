...
class ApproveAction(ActionProxy):
    def process_action(self):
        self.node.mark_approved(self)
    

    def cancel_action(self):
        self.node.mark_approved(None)
    
                
    def describe(self, viewer=None):
        return _("%(user)s approved %(post_desc)s") % {
            'user': self.hyperlink(self.user.get_profile_url(), self.friendly_username(viewer, self.user)),
            'post_desc': self.describe_node(viewer, self.node)
        }
...
class NodeViewAction(DummyActionProxy):
    def __init__(self, node, user, ip=None):
        self.viewuser = user
        self.node = node
        super(NodeViewAction, self).__init__(ip)
