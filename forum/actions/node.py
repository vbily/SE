...
from forum.models import Comment, Question, Answer, NodeRevision, Case, Article
...
class AddCaseAction(NodeEditAction):
    verb = _("added")
    ...
    def process_data(self, **data):
        ...
        case = Case(author=self.user, **processed_data)
        if self.user.is_superuser or self.user.is_staff:
            case.approved = True
            case.nis.approved = True
        else:
            case.approved = False
        case.save()
        self.node = case
    ...
    def describe(self, viewer=None):
        return _("%(user)s added %(case)s") % {
            'user': self.hyperlink(self.user.get_profile_url(), self.friendly_username(viewer, self.user)),
            'case': self.hyperlink(self.node.get_absolute_url(), self.node.title)
    }

class AddArticleAction(NodeEditAction):
    verb = _("added")
    ...
    def process_data(self, **data):
        ...
        article = Article(author=self.user, **processed_data)
        if self.user.is_superuser or self.user.is_staff:
            article.approved = True
            article.nis.approved = True
        else:
    ...
    def describe(self, viewer=None):
        return _("%(user)s added %(case)s") % {
            'user': self.hyperlink(self.user.get_profile_url(), self.friendly_username(viewer, self.user)),
            'article': self.hyperlink(self.node.get_absolute_url(), self.node.title)
}
