...
from forum.models import Question, Case, Term
...
def prepare_locations(section, sitemaps):
    locations = []
...
def index(request, sitemaps):
    ...
    locations.extend(prepare_locations('questions', sitemaps))
    locations.extend(prepare_locations('cases', sitemaps))
    locations.extend(prepare_locations('terms', sitemaps))
    locations.extend(prepare_locations('misc', sitemaps))
...
class CasesSitemap(OsqaSitemap):
    
    def items(self):
        return Case.objects.filter_state(deleted=False).order_by('id')
    

class TermsSitemap(OsqaSitemap):
    
    def items(self):
        return Term.objects.filter_state(deleted=False).order_by('title')
...
class MiscSitemap(Sitemap):
    ...
    def get_urls(self, page=1):
        urls = []
        url_info = {
            'location':   'http://www.seamenexchange.com/tags',
        }
        urls.append(url_info)
        url_info = {
            'location':   'http://www.seamenexchange.com/library',
        }
        urls.append(url_info)
        return urls
