...
def approve_post(request, id):
    ...
    ret = {'commands': {}}
    if post.nis.approved:
        post.nstate.approved.cancel(user, ip=request.META['REMOTE_ADDR'])
        ret['commands']['unmark_approved'] = [post.node_type, id]
    else:
        ApproveAction(node=post, user=user, ip=request.META['REMOTE_ADDR']).save()
        ret['commands']['mark_approved'] = [post.node_type, id]
