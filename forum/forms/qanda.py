...
class MarineKnowledgeClassifier:
        def __init__(self):
                self.classifier = myclassify()
                self.classifier.setWriteApiKey('J42r0FybyBxK')
                self.classifier.setReadApiKey('KQOiTh2yCzd1')

        def notMarineRelated(self, text):
		#m = str.maketrans('', '', u'\u201c\u201d')
                result = self.classifier.classify([text], "MarineKnowledge")
                ans_dict = dict([tup for tup in result[0][2] if tup[0] == 'NotRelated'])
                return float(ans_dict['NotRelated']) > 0.25

        def isEnglish(self,text):
            #m = str.maketrans('', '', u'\u201c\u201d')
            result = self.classifier.classify([text], "Text Language", "uClassify")
            ans_dict = dict([tup for tup in result[0][2] if tup[0] == 'English'])
            return float(ans_dict['English']) > 0.5        
...
class TitleField(forms.CharField):
	...
	#if antispam.is_spam(value):
        #        raise forms.ValidationError(_('spam'))
	classifier = MarineKnowledgeClassifier()
       	#if not classifier.isEnglish(value):
	#	raise forms.ValidationError(_('SeamenExchange language is English only. Please rephrase.'))
    #if classifier.notMarineRelated(value):
    #            raise forms.ValidationError(_('question not related to Marine industry. Please rephrase.'))
        return value
...
class CaseEditorField(EditorField):
    def __init__(self, *args, **kwargs):
        super(CaseEditorField, self).__init__(*args, **kwargs)
        self.required = not bool(settings.FORM_EMPTY_QUESTION_BODY)
    
    
    def clean(self, value):
        super(CaseEditorField, self).clean(value)
...
class ArticleEditorField(EditorField):
    def __init__(self, *args, **kwargs):
        super(ArticleEditorField, self).__init__(*args, **kwargs)
        self.required = not bool(settings.FORM_EMPTY_QUESTION_BODY)
    
    
    def clean(self, value):
        super(ArticleEditorField, self).clean(value)
...
class AddCaseForm(forms.Form):
    ...
    def __init__(self, data=None, user=None, *args, **kwargs):
        super(AddCaseForm, self).__init__(data, *args, **kwargs)
...
class AddArticleForm(forms.Form):
    title  = TitleField()
    text   = ArticleEditorField()
    
    def __init__(self, data=None, user=None, *args, **kwargs):
        super(AddArticleForm, self).__init__(data, *args, **kwargs)
...
class EditCaseForm(forms.Form):
    title  = TitleField()
    text   = CaseEditorField()
    summary = SummaryField()
    
    def __init__(self, case, user, revision=None, *args, **kwargs):
        super(EditCaseForm, self).__init__(*args, **kwargs)
        
        if revision is None:
            revision = case.active_revision
