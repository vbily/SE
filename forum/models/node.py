...
class Node(BaseModel, NodeContent):
    ...
    def mark_approved(self, action):
        self.nstate.approved = action
        self.approved = not (action is None)
        self.save()
    ...
    def save(self, handle_revision = True, handle_activity = True, *args, **kwargs):
        if not self.id:
